...
The `EmbeddingService` (`storage/embedding_service.py`) is a thin ONNX wrapper: synchronous `encode()` for CPU-bound inference, async `save_by_vector` / `search_by_vector` for pgvector upsert/search. Tool code encodes via `asyncio.to_thread` before acquiring a DB connection so the pool is never pinned during inference. Reindex is a library function (`tools/admin._run_reindex`) used by future admin APIs -- it is not registered as an MCP tool, since the `tsvector` index is always current and only the embedding side is ever rebuilt.
