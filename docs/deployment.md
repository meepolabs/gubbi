...
| `JOURNAL_EXTRA_ALLOWED_ORIGINS` | no | Comma-separated extra origins accepted on `/mcp` (DNS-rebinding guard), added to the built-in hosted MCP client origins (`https://claude.ai`, `https://chatgpt.com`). Loopback is always allowed. Each entry must be a bare `http(s)://host[:port]` -- a path, wildcard, or malformed entry fails startup. | `https://journal.example.com,https://mcp.example.com` |
