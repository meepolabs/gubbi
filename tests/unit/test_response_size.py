...
from gubbi.tools.response_size import assert_response_ok

pytestmark = pytest.mark.unit


def test_ok_payload_returns_none() -> None:
    result = assert_response_ok({"entries": [{"id": "1"}]})
    assert result is None


def test_large_payload_returns_error_dict() -> None:
    huge = {"entries": [{"content": "x" * 100_000}]}
    result = assert_response_ok(huge)
...
def test_custom_threshold() -> None:
    payload = {"x": "a" * 200}
    assert assert_response_ok(payload, error_threshold_chars=100) is not None
    assert assert_response_ok(payload, error_threshold_chars=10_000) is None
...
def test_unserializable_passes_through() -> None:
    ...
    result = assert_response_ok({"obj": NotSerializable()})
    assert result is None


def test_simple_payload_fits_threshold() -> None:
    small = {"msg": "hello", "count": 42}
    result = assert_response_ok(small)
    assert result is None
...
def test_conversations_too_large_returns_error() -> None:
    ...
    result = assert_response_ok(huge, tool_name="journal_list_conversations")
    assert result is not None
    assert result["error"] == "response_too_large"
    assert "current_size_chars" in result
    assert "suggested_max_limit" in result
...
def test_conversations_normal_fits() -> None:
    ...
    result = assert_response_ok(small, tool_name="journal_list_conversations")
    assert result is None
...
def test_topics_too_large_returns_error() -> None:
    ...
    result = assert_response_ok(huge, tool_name="journal_list_topics")
    assert result is not None
    assert result["error"] == "response_too_large"
    assert "current_size_chars" in result
    assert "suggested_max_limit" in result
...
def test_topics_normal_fits() -> None:
    ...
    result = assert_response_ok(small, tool_name="journal_list_topics")
    assert result is None
