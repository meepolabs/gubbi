...
Pins the invariant: ``APP_POOL_SIZE_MAX`` must remain large enough to
...
def test_app_pool_max_exceeds_worker_max_jobs() -> None:
    ...
    extraction path (see gubbi/constants.py for the rationale).
    """
    headroom = 2
    ...
    assert minimum_required <= APP_POOL_SIZE_MAX, (
        ...
        "see gubbi/constants.py:18 for the rationale."
    )
