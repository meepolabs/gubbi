...
| `storage/` | `pg_setup.py` (asyncpg pool init + advisory locks), `embedding_service.py` (ONNX MiniLM-L6-v2 + pgvector), `repositories/` (all SQL: topics, entries, conversations, search, plus `stats.py` dashboard aggregates and `setup_signals.py` onboarding EXISTS probes; topics carries the rename/delete-with-reassign/merge mutations and entries carries `list_entries`/`get_entry_by_id`/`move_entries_to_topic` for the web surface). Repo functions take `conn: asyncpg.Connection` first, encrypt/decrypt via injected cipher. |
| `services/` | Stateless query/composition layer reused across surfaces. `search.py` (`run_journal_search` -- the hybrid FTS+semantic pipeline extracted from the MCP search tool so the tool and the web `/api/v1/search` endpoint share one implementation). |
| `models/` | Plain dataclasses: `TopicMeta`, `Entry`, `Message`, `ConversationMeta`, `SearchResult`. |
| `tools/` | MCP tool handlers grouped by surface. Each module exports `register(mcp, app_ctx)`. `registry.py` calls them in order: topics -> entries -> search -> conversations -> context. `context.py` (`journal_briefing`) emits `BRIEFING_SEED_INSTRUCTIONS` (constants.py) on a truly-empty journal so the calling LLM auto-onboards, and exports the public `resolve_period(period, today)` -- the authority for the period vocabulary, consumed by `storage/repositories/stats.py`, `scripts/dump_period_fixtures.py`, and (re-implemented in TypeScript, pinned to that generator's fixture) the web client. `admin.py` is a library function (not registered), used by future admin API. |
| `api/v1/` | REST endpoints reached via the cloud-api gateway (HMAC `X-Auth-*` envelope verified by `auth.require_scope`). `auth.py` (scope dependency), `ingest.py`, `extraction.py`, `user.py` (`/user/setup-signals` onboarding booleans), and `web/` -- the web read/write REST API (`topics`, `entries` + `entries_admin`, `conversations`, `search`, `stats`, `timeline`, `topic_admin`) sharing `schemas.py` (`PaginatedList`, `DecryptableItem`), `decryption.py` (`decrypt_field` sentinel path -- a corrupt row yields `[decryption failed]` + a flag, never a 500), `pagination.py`, `responses.py` (`private, no-store`), and `errors.py`. Reads require `journal:read`, mutations `journal:write`. |
...
| `telemetry/` | OTel span/metric/log helpers. `attrs.py` (canonical attribute names including `SpanNames.EXTRACTION_JOB` + `SpanNames.EXTRACTION_LLM_CALL`), `metrics.py` (Prometheus counters/histograms + `rebind_metrics_after_configure` hook), `spans.py` (gubbi-common allowlist wrapper), `logger.py` (structlog -> OTel logs bridge), `sanitized_errors.py` (`record_exception_sanitized` / `safe_error_fields` / `is_driver_caused` -- describe a database failure by exception type + shape-validated SQLSTATE so a driver message or DETAIL block never reaches a span or log line; classification walks a bounded `__cause__`/`__context__` chain so a wrapper like `DatabaseUnavailable` is caught too), `asyncpg_sanitized.py` (`instrument_asyncpg_sanitized` wraps `AsyncPGInstrumentor`'s tracer so its per-statement CLIENT spans record a bounded `exception` event instead of the driver's message/stacktrace; wired from `_wire_instrumentors`, and FAIL-CLOSED -- if the tracer cannot be replaced the instrumentation is removed and verified gone, raising `SanitizerUnsafeError` if a wrapper survives, so the process never runs with default leaking spans). Three "orphan" counters (`orphan_cleanup`, `extract_conversation`, `anthropic_provider`) use `@lru_cache` factories deferred to first call; `rebind_metrics_after_configure` clears and re-primes all four counter factories after lifespan `configure_otel` so they bind to the SDK provider, not the import-time NoOp. Adding a new orphan counter requires wiring BOTH this rebind hook AND the autouse test fixture. |
